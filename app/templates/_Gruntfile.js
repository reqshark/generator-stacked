module.exports = function(grunt) {
  ...
  grunt.loadNpmTasks('grunt-shell');<% if(less) { %>
  grunt.loadNpmTasks('grunt-contrib-less');<% } if (mocha) { %>
  grunt.loadNpmTasks('grunt-mocha-test');<% } %>

  grunt.registerTask('test', ['jshint'<% if (mocha) { %>, 'mochaTest' <% } %>]);
  grunt.registerTask('init', ['shell:copyBootstrapCSS', 'shell:copyFontAwesomeCSS', 'shell:copyFontAwesomeFonts',<% if (less) { %>'less:production', <% } %>'requirejs:mainJS', 'requirejs:mainCSS']);
  grunt.registerTask('build', [<% if (less) { %>'less:production', <% } %>'requirejs:mainJS', 'requirejs:mainCSS']);
